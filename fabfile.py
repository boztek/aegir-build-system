...
def _provision_new_site(site_uri, platform_id, app_id, db_server_id, email):
    ...
    local('php /var/aegir/drush/drush.php provision-save @%s --context_type=site --uri=%s --platform=@platform_%s --db_server=@server_%s --client_email=%s --profile=%s' % (site_uri,site_uri,platform_id,db_server_id,email,app_id,site_uri))
